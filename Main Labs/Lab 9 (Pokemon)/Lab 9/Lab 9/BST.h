...
public:
	...
	//Returns an in order alphabatized version of the BST
	string toStringAlphabetical() {
		std::stringstream out;
		if (root_ == NULL) out << "EMPTY YOU CHUMP!";
		else {
			toStringAlphabeticalRecurssive(root_, out);
		}
		return out.str();
	}
	
	//Returns true if the node's data was added, and false if it is NULL. Called recursively by toStringAlphabetical();
	void toStringAlphabeticalRecurssive(Node*& root, std::stringstream& out) {
		if (root == NULL) {
			return;
		}
		toStringAlphabeticalRecurssive(root->left_, out);
		if (root->data_ == "RECOGNIZE ME") {
			out << "";
		}
		else {
			out << " " << root->data_;
		}
		toStringAlphabeticalRecurssive(root->right_, out);
	}
	...
	bool findNode(const T& data) { return find(root_, data); }

	//Return true if the element exists in the BST. Is called recursively.
